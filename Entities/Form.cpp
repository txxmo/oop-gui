...
void Form::Draw()
{
    ...
    if (!IsMouseInTriangle(StretchPoint1, StretchPoint2, StretchPoint3) && IsMouseInRectangle(Form::Pos, Form::Size) && GetCurrentCursor() == Cursors["Corner Drag"] && !Form::Stretching)
        SetCurrentCursor(LIT("Default"));
    if (IsMouseInTriangle(StretchPoint1, StretchPoint2, StretchPoint3) && Form::CanStretch)
        SetCurrentCursor(LIT("Corner Drag"));
  
    OutlineRectangle((Form::Pos.x - Form::Border.x / 2)+1, (Form::Pos.y - Form::Border.x / 2)+1, Form::Size.x + Form::Border.x-2, Form::Size.y + Form::Border.x-2, 1, Colour(100, 185, 255, 200)); // Draw Border
  ...
  //  FilledRectangle(Form::Pos.x, Form::Pos.y, Form::Size.x, Form::Border.y, Colour(30, 30, 30, 255)); // header
    ...
    DrawText(Form::Pos.x + 5, Form::Pos.y + 5, Form::Name, LIT("Verdana"), 12, Colour(255, 255, 255, 255), None);
    if (Form::CanStretch)
